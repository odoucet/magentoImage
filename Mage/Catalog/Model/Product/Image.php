...
//        print '$this->_getNeedMemoryForBaseFile() = '.$this->_getNeedMemoryForBaseFile();
	...
	*/
	...
	/* OD
        ...
        $miscParams = array(
                ($this->_keepAspectRatio  ? 'kar=1' : '') ,
                ($this->_keepFrame        ? 'kf=1' : '')  ,
                ($this->_keepTransparency ? 'kt=1' : '')  ,
                ($this->_constrainOnly ?    'co=1' : '') ,
                'bg='.$this->_rgbToString($this->_backgroundColor),
                'angle=' . $this->_angle,
        ...
        if ($this->getWatermarkFile()) {
            $miscParams[] = 'wf='.$this->getWatermarkFile();
            $miscParams[] = 'wio='.$this->getWatermarkImageOpacity();
            $miscParams[] = 'wp='.$this->getWatermarkPosition();
            $miscParams[] = 'ww='.$this->getWatermarkWidth();
            $miscParams[] = 'wh='.$this->getWatermarkHeigth();
        }

        $this->_newFile = implode('/', $path) . $file.'?'.implode(';', $miscParams); // the $file contains heading slash
